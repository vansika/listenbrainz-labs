...
Model = namedtuple('Model', 'model rank lmbda iteration model_id training_time')

def parse_dataset(row):
    return Rating(row['user_id'], row['artist_id'], row['count'])
...
def preprocess_data(playcounts_df):
    print("\nSplitting dataframe...")
    training_data = playcounts_df.rdd.map(parse_dataset)
    return training_data

def train(training_data, rank, lmbda, iteration):
    best_model = None
    best_model_metadata = {}
    alpha = 3.0 
    t0 = time.time()
    ...
    model_id = 'listenbrainz-recommendation-model'
    best_model = Model(model=model, rank=rank, lmbda=lmbda, iteration=iteration, model_id=model_id, training_time=mt)
    best_model_metadata = {'rank': best_model.rank, 'lmbda': best_model.lmbda, 'iteration': best_model.iteration, 
                    'model_id': best_model.model_id, 'training_time' : best_model.training_time}
    return model, best_model_metadata
...
def main():
    ...
    try:
        path = os.path.join('/', 'data', 'listenbrainz', 'artist-similarity', 'dataframes', 'playcounts_df.parquet')
    ...
    t0 = time.time()
    training_data = preprocess_data(playcounts_df)
    ...
    for attempt in range(config.MAX_RETRIES):
        try:
            t0 = time.time()
            model, best_model_metadata = train(training_data,12, 0.1, 20)
    ...
    for attempt in range(config.MAX_RETRIES):
        try:
            t0 = time.time()
            path = os.path.join('/', 'data', 'listenbrainz', 'artist-similarity', 'best_model', '{}'.format(best_model_metadata['model_id']))
            model.save(listenbrainz_spark.context, config.HDFS_CLUSTER_URI + path)
            t = "%.2f" % ((time.time() - t0) / 60)
